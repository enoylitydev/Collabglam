...
const isUnlimited = (k: string, v: FV) =>
  v === Infinity ||
...
const formatValue = (key: string, v: FV): string => {
  ...
  if (key === "support_channels" && Array.isArray(v)) {
    return v
      .map(
        (s) =>
          SUPPORT_PRETTY[String(s).toLowerCase()] ?? String(s)
      )
      .join(" + ");
  }

  if (key === "media_kit" && typeof v === "string") {
    if (v === "included_standard") return "Included (Standard)";
    if (v === "included") return "Included";
    return nice(v);
  }

  if (key === "team_manager_tools_managed_creators" && v && typeof v === "object") {
    const { min, max } = v as any;
    if (min != null && max != null) return `Manage ${min}–${max} creators`;
  }
...
const Pricing: React.FC = () => {
  ...
  const orderedPlans = useMemo(() => {
    ...
    return plans.map((p) => {
      ...
      return {
        ...p,
        _ordered: [...known, ...remaining] as Feature[],
      };
    });
  }, [plans, activeRole]);

  const handleSelect = async (plan: Plan) => {
    if (isEnterpriseBrand(plan)) {
      router.push("/contact-us");
      return;
    }
    try {
      const r = await post<{ checkoutUrl?: string }>(
  ...
  return (
    <section
      id="pricing"
      className="relative py-20 bg-gray-50 font-lexend"
    >
