...
/** ==== Types aligned to server (parity with Brand) ==== */
type ReplySnapshot = {
  messageId: string;
  senderId: string;
  text: string;
...
const CHAR_LIMIT = 2000; // brand parity
...
const msgKey = (
  m: Pick<Message, "senderId" | "timestamp" | "text" | "messageId">
...
export default function ChatWindow({ params }: { params: { roomId: string } }) {
  ...
  useEffect(() => {
    ...
    post<{ rooms: any[] }>("/chat/rooms", { userId: influencerId })
      .then(({ rooms }) => {
        ...
        const other = room?.participants?.find((p: any) => p.userId !== influencerId);
  ...
  useEffect(() => {
    ...
    if (influencerId && roomId) {
      post("/chat/mark-seen", { roomId, userId: influencerId }).catch((err) =>
        console.error("Failed to mark messages as seen:", err)
      );
    }
  }, [loadHistory, influencerId, roomId]);
  ...
  useEffect(() => {
    ...
    const url = process.env.NEXT_PUBLIC_WS_URL || "ws://infuencer.onrender.com/ws"; // parity with Brand fallback
  ...
  /** ===== Attachment handling (parity with Brand) ===== */
  const onAttachClick = () => {
  ...
  const sendFiles = async (files: FileList) => {
    if (!files || files.length === 0 || !influencerId) return;

    const form = new FormData();
    form.append("roomId", roomId);
    form.append("senderId", influencerId);
  ...
  /** ====== UI (WhatsApp-like, Influencer amber theme) ====== */
  const sentBubble =
    "bg-[#FFF6CC] text-gray-900 rounded-2xl rounded-br-none shadow"; // soft yellow tint
  const recvBubble =
  ...
  return (
    <Card className="relative flex h-screen flex-col min-h-0">
      ...
      {/* BODY — pale yellow background */}
      <div className="flex-1 flex flex-col min-h-0 bg-[#FFFBEA]">
              ...
              {messages.map((raw, idx) => {
                ...
                const ringHighlight =
                  highlightId === msg.messageId
                    ? "ring-2 ring-amber-300 ring-offset-2 ring-offset-[#FFFBEA]"
                    : "";
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col items-stretch max-w-[90%] sm:max-w-[70%]">
                      ...
                      {msg.reply && (msg.reply.text || msg.reply.hasAttachment) && (
                        <button
                          ...
                          className="text-left mb-1 border-l-4 border-amber-400 bg-white/80 px-3 py-2 rounded-md hover:bg-white transition-colors"
                          title="Go to original"
                        >
                          <div className="text-[11px] font-semibold text-amber-700">Replying to</div>
                    ...
                    {isMe && (
                      <Avatar className="h-7 w-7 ml-2 self-end hidden sm:inline-flex border-2 border-amber-300">
                        <AvatarFallback className="text-[10px] text-gray-800">Me</AvatarFallback>
                      </Avatar>
                    )}
                  </div>
                );
        ...
        {/* INPUT — WhatsApp layout, influencer gradient action */}
        <CardFooter className="bg-[#FFF6DC] border-t px-3 sm:px-6 py-3 sm:py-4">
          <div className="w-full">
            {replyTo && (
              <div className="mb-2 flex justify-between items-center bg-white px-3 sm:px-4 py-2 rounded-lg shadow-sm border-l-4 border-amber-400">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <button
                ...
                className="p-2 rounded-full text-amber-600 hover:bg-white"
              ...
              <Textarea
                ...
                rows={1}
                disabled={uploading}
                className="flex-1 resize-none bg-white focus:ring-2 focus:ring-amber-400 rounded-full px-4 py-2"
              />

              <Button
                size="icon"
                className="bg-gradient-to-r from-[#FFBF00] to-[#FFDB58] text-gray-800 hover:opacity-90 rounded-full disabled:opacity-60"
      ...
      {error && (
        <p className="absolute bottom-0 w-full text-center text-xs text-red-500 pb-2">{error}</p>
      )}
    </Card>
  );
}
