...
interface ApiMeta {
  ...
  page: number;
  limit: number;
  totalPages?: number; // backend may send this
  pages?: number; // or this
}
...
export default function BrandPreviousCampaignsPage() {
  ...
  const [limit] = useState(10); // keep constant, expose if needed
  ...
  // ---- Fetch from backend with search/page/limit ----
  const fetchCampaigns = useCallback(
    async (page: number, term: string) => {
      ...
      try {
        ...
        const res = await get<CampaignsApiResponse>("/campaign/previous", {
          brandId,
  ...
  // Optional: still keep this logic if you show influencer counts lazily
...
interface TableViewProps {
  ...
  onToggle: (c: Campaign) => Promise<void> | void;
    ...
    <div className="p-[1.5px] rounded-lg bg-gradient-to-r shadow">
      <div className="overflow-x-auto bg-white rounded-[0.5rem]">
        <table className="w-full text-sm text-gray-600">
          ...
          >
            <tr>
              {["Campaign", "Budget", "Status", "Timeline", "Influencers Applied", "Actions"].map((h) => (
          ...
          <tbody>
            {data.map((c, idx) => (
              <tr
                key={c.id}
                className={`${idx % 2 === 0 ? "bg-white" : "bg-gray-50"} group transition-colors hover:bg-transparent`}
                ...
                <td className="px-6 py-4 whitespace-nowrap align-top">
                  <div className="flex items-center space-x-2">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Link
                          href={`/brand/prev-campaign/applied-inf?id=${c.id}&name=${encodeURIComponent(
